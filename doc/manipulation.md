...
例えば、"A".compareTo("B")の場合は、compareToメソッドを理解する上で便宜上式のcompareToを「-」に置換すると、"A" - "B"となりますが、"A"より"B"の方がChar順で後ろにあるため"A" - "B" &lt; 0です。この「-」をcompareToに元に戻すと、"A".compareTo("B") &lt; 0になりますが、これはtrueです。
